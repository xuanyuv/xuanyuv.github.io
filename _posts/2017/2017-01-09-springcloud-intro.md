...
Spring Cloud Config 为服务端和客户端提供了分布式系统的外部化配置支持，即它为各应用的所有环境提供了一个中心化的外部配置

说白了就是：各个应用（无论服务端还是客户端）针对研发中的每套环境，可能都有不同的配置，在发布环境时都要注意所使用的配置文件

而通过 Spring Cloud Config 实现的配置中心，能够达到所有应用的所有环境的配置文件统一集中管理，且支持热更新

它默认采用 git 存储配置信息，这有助于对环境配置进行版本管理，并且通过 git 客户端工具也能够很方便的管理和修改配置内容

...
而是在阿里云网页后台，安全组里配置内网端口（1400）允许被跳板机（192.168.1.1）访问

实际发现：在跳板机上，执行命令 **curl http://192.168.0.1:1400/** ，会有如下提示

```text
curl: (7) Failed connect to 192.168.0.1:1400; No route to host
```

连跳板机都访问不通，更不用说在浏览器里走代理访问了

后面在内网机器防火墙里直接加端口（同时阿里云安全组里删掉 1400 端口的跳板机授权访问）

此时发现：跳板机上执行 **curl http://192.168.0.1:1400/** 也是通的了，浏览器走代理也成功了
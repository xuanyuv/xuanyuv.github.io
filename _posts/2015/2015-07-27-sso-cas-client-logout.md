...
与单点登录相对应：通过CASServer登出所有的CASClient，登录的URL是`/login`，登出的URL是`/logout`

这里需配置SingleSignOutFilter（用来使Session失效）和SingleSignOutHttpSessionListener（用于Session过期时移除其对应映射关系）

1、要为SingleSignOutFilter配置casServerUrlPrefix参数

2、默认的，登出后会跳转到CASServer的登出页，若想跳转到其它资源，可在`/logout`的URL后面加上`service=you want to jump url`

　　比如`http://sso.jadyer.com:8080/cas-server-web/logout?service=http://blog.csdn.net/jadyer`
...
3、禁用单点登出：`CASServer/WEB-INF/cas.properties`中的`slo.callbacks.disabled=true`
...
　　或者登出后，点浏览器后退按钮，发现会后退到之前的CASClient资源页，但在这个页面点击任何请求，都会自动重定向到单点登录页

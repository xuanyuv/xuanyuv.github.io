...
这里也用 YAML 文件，所以创建一个 frpc.yaml（注意：这里是 frp**C**.yaml，不是 frp**S**.yaml）
...
也可以安装成 Win10 服务，步骤和这个一样：<https://www.xuanyuv.com/blog/20130908/centos-develop.html#安装Win10服务>
...
下面介绍一下，如何实现连接远程 Windows（使用起来很丝滑，一点不输向日葵、ToDesk、TeamViewer）

服务端不用改，还是用上面的配置，客户端的话，出于安全考虑，采用 **sctp** 的方式

> * tcp 的方式，不需要 secretKey 就能连，很不安全
* xtcp 的方式，不同的宽带网络下，不一定每次都能点对点穿透成功

由于是 stcp 的方式，那么对于客户端而言，要细分为 控制端（本地 Windows） 和 被控端（远程 Windows）
...
proxies:
  - name: rdp-remote-xuanyuv   # 被控端名称
    type: stcp
    secretKey: nothingNoMatter # 被控端密钥
    localIP: 127.0.0.1
    localPort: 3389
    transport:
...
visitors:
  - name: rdp-local-xuanyuv
    type: stcp
    serverName: rdp-remote-xuanyuv # 被控端名称
    secretKey: nothingNoMatter     # 被控端密钥
    bindAddr: 127.0.0.1
    bindPort: 8389
```

启动 被控端 和 控制端，远程桌面连接：`127.0.0.1:8389` 即可（被控端需要在控制面板启用远程桌面）

如果服务端是一台 3M 的服务器，那可用带宽最多 `3M = 3 * 128KB/s = 384KB/s`，**实际也就 300KB/s 左右**

所以远程桌面连接时，默认会卡的界面一动不动，此时像下面这样设置，便可纵享丝滑了：

* 远程桌面连接 --- 显示 --- 显示配置 --- 调整显示分辨率（一般默认全屏就行，除非下面的设置完还卡，再来调小这个）

...
运行---msinfo32---左侧组件---存储---磁盘，将右侧所有**分区起始偏移**，每个值去除4096，结果为整数表示4K对齐，小数则没对齐
...
打开 HWIDGen，右上角工作模式选择 **KMS38激活**（也就是激活到2038年），然后左下角开始按钮，等待一分钟左右，激活成功
...
2. 有兴趣根据 MSDN 母盘精简系统组件，并初始化系统配置，再封装成 iso 的，可尝试 NTLite 或 MSMG Toolkit（二者各有优点）
...
JPA规范要求类路径下放置一个文件名固定的`//META-INF//persistence.xml`文件，如下所示
...
下面是采用`Annotation`映射元数据的实体类

其中，注解可以标注在属性或者`getter`上面，但是：要么全标注在属性上，要么全标注在getter上

比如下面实体类中配置的那样，如果把`@Id`标注在了属性上面，那就没有必要再把注解标注在其它属性的getter上面了

因为当发现`@Id`标注在属性上，它就会认为所有注解都标准在属性上，于是会忽略对getter的检查
...
/**
 * Created by 玄玉<https://jadyer.github.io/> on 2010/11/22 17:26.
 */

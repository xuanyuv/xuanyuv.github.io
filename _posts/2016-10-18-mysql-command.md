...
SELECT t.tag 渠道, count(*) 今日扫描量 FROM t_qq_qrcode t WHERE datediff(now(),t.create_time)=0 GROUP BY t.tag;
SELECT t.tag 渠道, count(*) 昨日扫描量 FROM t_qq_qrcode t WHERE datediff(now(),t.create_time)=1 GROUP BY t.tag;
SELECT t.tag 渠道, count(*) 本周扫描量 FROM t_qq_qrcode t WHERE yearweek(date_format(t.create_time,'%Y-%m-%d'))=yearweek(now()) GROUP BY t.tag;
SELECT t.tag 渠道, date_format(t.create_time, '%Y%m%d') 日期, count(*) 当日扫描量 FROM t_qq_qrcode t WHERE date_format(t.create_time, '%Y%m%d')='20160503' GROUP BY t.tag;
```

## 表中存在重复数据时的统计
```sql
-- 對於表中存在重複數據的，查詢出過濾掉重複數據后的
SELECT id FROM coop_push_user GROUP BY mobile HAVING count(mobile)=1;
-- 對於表中存在重複數據的，查詢重複數據中最舊的那條
SELECT id FROM coop_push_user GROUP BY mobile HAVING count(mobile)>1;
-- 對於表中存在重複數據的，查詢重複數據中最新的那條，對於其它無重複數據的則原樣查出
SELECT mobile, status-1, create_time FROM coop_push_user WHERE id in(SELECT max(id) FROM coop_push_user GROUP BY mobile);
```
...
而你的“操作”通常是insert/update/delete操作，这些操作又都是当前读，也就是拿数据库最新数据来操作
...
>严格来讲，是临键锁（Next-key Lock，由行锁加间隙锁构成）解决了幻读
...
接着，引擎层再把下一条记录加锁返回给Server层，重复相同的过程，最终所有记录都加上锁都更新了，接下来就等待提交

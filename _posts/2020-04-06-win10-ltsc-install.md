...
虽然修改注册表的方式可以关闭，但是 Win10 下会导致电脑右键管理，弹出对话框：该文件没有与之关联的应用来执行该操作balabala

虽然如此，我们还是可以通过：运行---compmgmt.msc 来打开计算机管理面板
...
打上无线网卡驱动，联网后，Win10 会自动扫描硬件，并安装相应的驱动

不过它自动装的驱动可能不是最新的，还是自己提前到品牌电脑官网下载驱动，自己安装完，再联网

我们也可以关闭它自动装驱动的功能，有下面几种方法（因为涉及到 WindowsUpdate，所以不保证一定好使）

1. 此电脑---右键属性---高级系统设置---硬件--设备安装设置---否
2. 运行---gpedit.msc---计算机配置---管理模板---Windows组件---Windows更新---双击右侧 Windows 更新不包括驱动程序---更改为已启用
3. 运行---gpedit.msc---计算机配置---管理模板---系统---设备安装---设备安装限制---双击右侧禁止安装未由其他策略设置描述的设备---更改为已启用
4. 运行---gpedit.msc---计算机配置---管理模板---系统---设备安装---设备安装限制---双击右侧阻止使用与下列设备安装程序类相匹配的驱动程序安装设备
　　<br/>更改为已启用，然后点击下面的【显示】按钮，输入设备的GUID值，确定后再勾选也适用于匹配已安装的设备

找设备的GUID值：电脑右键管理打开设备管理器，展开一个设备，右键属性，详细信息选项卡，属性下拉框选择类Guid，右键复制下面显示的值，即可
